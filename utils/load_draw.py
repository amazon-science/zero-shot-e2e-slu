...
def multi_view_filter_process(
    ...
    ### diverse from multi-view
    # if 'multi_view_balance' not in hparams.keys():
    #     hparams['multi_view_balance'] = False
    # if hparams['multi_view_balance'] == True:
    #     print('------------------------------------------------')
    #     print('3.2 Process to balance from three views')
        ...
        # # draw bar in terms of #samples in each text cluster
        # balance_gt_txt2transcipt_div_score, balance_pr_txt2transcipt_div_score = dmv.draw_overlapped_bar(
        #     gt_data_array=sample_label_array_cluster,
        #     pr_data_dict=chose_text2transcript_cluster_id,
        #     view_key_word='num_sample_T2_text_cluster',
        #     # add save pdf name
        #     save_name='balance'
        # )
        #
        # # draw bar in terms of #samples in each audio cluster
        # balance_gt_adu2wav_div_score, balance_pr_adu2wav_div_score = dmv.draw_overlapped_bar(
        #     gt_data_array=aud_sample_label_array_cluster,
        #     pr_data_dict=chose_adu2wav_cluster_id,
        #     view_key_word='num_sample_aux_audio_cluster',
        #     save_name='balance'
        # )
        #
        # # draw bar in terms of #samples in each semantics
        # balance_gt_text2sem_div_score, balance_pr_text2sem_div_score = dmv.draw_overlapped_bar(
        #     gt_data_array=processed_sample_semantic_list,
        #     pr_data_dict=chose_text2sem_cluster_id,
        #     view_key_word='num_semantics_cluster',
        #     save_name='balance'
        # )
        #
        # total_balance_gt_div_score = (
        #                                          balance_gt_txt2transcipt_div_score + balance_gt_adu2wav_div_score + balance_gt_text2sem_div_score) / 3
        # total_balance_pr_div_score = (
        #                                          balance_pr_txt2transcipt_div_score + balance_pr_adu2wav_div_score + balance_pr_text2sem_div_score) / 3
        # print(
        #     f'basic sf has total gt_div_score and total pr_div_score as {total_balance_gt_div_score} and {total_balance_pr_div_score}.')



# ## below is the copy of original draw and multi-view SF
# # draw diagrams for the SM results
#     if 'draw_img' not in hparams:
#         hparams['draw_img'] = False
#     if hparams['draw_img']:
#         print('start draw diagrams from the multi-views')
#         print(final_filter_aux_sample_ids.item().keys())
#         cluster_metrics = ['text_fea', 'sem_dis', 'aud_div']
#
#
#         # load T2 text cluster results
...
#                 semantic_info = json.loads(semantic_info)
#                 sem_type_list = semantic_info['semantic_keywords']
#                 sample_semantic_list = semantic_info['sample_semantic_list'] # will be greater than #sample, because some samples have more than one semantics
#
#
#
#         # load sample_filter res
...
#     if hparams['multi_view_balance'] == True:
#         print('------------------------------------------------')
#         print('3.2 Process to diverse from three views')
#
#         # load semantic class centers
#         semantic_cls_fea_list = []
#         with open(hparams['semantic2vec_fea_file'], encoding='utf-8') as f:
...
#         # cal balanced sample ids:
#         balanced_min_sample_num_in_all_clustets, balanced_selected_sample_dataset_id, chose_text2transcript_cluster_id, chose_adu2wav_cluster_id, chose_text2sem_cluster_id = \
#             multi_view_sf.cal_balanced_sample_ids()
...
#         )
#
#         total_balance_gt_div_score = (balance_gt_txt2transcipt_div_score + balance_gt_adu2wav_div_score + balance_gt_text2sem_div_score)/3
#         total_balance_pr_div_score = (balance_pr_txt2transcipt_div_score + balance_pr_adu2wav_div_score + balance_pr_text2sem_div_score) / 3
#         print(f'basic sf has total gt_div_score and total pr_div_score as {total_balance_gt_div_score} and {total_balance_pr_div_score}.')
#
#         assert 1 == 0